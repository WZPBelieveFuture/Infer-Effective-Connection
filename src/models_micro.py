...
class LocalGroupEncoder(nn.Module):
    def __init__(self, macro_dim, group_sizes, hidden_units, num_layers, encoder_type="mlp", device=None):
        super().__init__()
        ...
        if len(self.group_sizes) != self.macro_dim:
            raise ValueError(
                f"macro_dim must equal the number of macro groups={len(self.group_sizes)}, but got {self.macro_dim}"
            )
    ...
    def decode(self, z):
        if z.dim() == 1:
            z = z.unsqueeze(0)
        if z.shape[1] != self.num_groups:
            raise ValueError(f"Expected macro latent dimension {self.num_groups}, but got {z.shape[1]}")

        recon = []
        if self.encoder_type == "invertible":
            for idx, flow in enumerate(self.group_flows):
                recon.append(flow.decode(z[:, idx : idx + 1]))
        else:
            for idx, decoder in enumerate(self.decoders):
                recon.append(decoder(z[:, idx : idx + 1]))
...
def _save_history_files(history, ei_value, ei_term1, ei_term2, forward_scale_loss, result_dir, scale_id, train_stage):
    ...
    history_df.to_csv(os.path.join(result_dir, "train_history_scale.csv" if int(train_stage) == 1 else f"train_history_scale{scale_id}.csv"), index=False)

    train_loss_path = os.path.join(
        result_dir,
        "train_loss.csv" if int(train_stage) == 1 else f"train_loss{scale_id}.csv",
    )
    test_loss_path = os.path.join(
        result_dir,
        "test_loss.csv" if int(train_stage) == 1 else f"test_loss{scale_id}.csv",
    )
    ...
    if int(train_stage) == 2:
        pd.DataFrame(np.array(ei_value)).to_csv(os.path.join(result_dir, f"EI_scale{scale_id}.csv"), index=False, header=False)
        pd.DataFrame(np.array(ei_term1)).to_csv(os.path.join(result_dir, f"term1_scale{scale_id}.csv"), index=False, header=False)
        pd.DataFrame(np.array(ei_term2)).to_csv(os.path.join(result_dir, f"term2_scale{scale_id}.csv"), index=False, header=False)
...
def train_and_memorize(
    ...
    stage1_model_dir = os.path.join(".", "loc_model_stage1", "stage1_macro")
    stage2_model_dir = os.path.join(".", "loc_model_stage2", str(run_name))
    ...
    for epoch in range(int(epoches)):
        ...
        if epoch % 500 == 0:
            ...
            model_name = "model.pkl" if int(train_stage) == 1 else f"model_scale{scale_id}.pkl"
